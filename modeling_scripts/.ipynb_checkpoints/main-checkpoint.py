...
def by_fold(k_ct):
    ...
    # Remove outliers
    train_set = data.remove_outliers(train_set)
    val_set = data.remove_outliers(val_set)
    # Organize dataframes
    df_TRAIN = data.organize_dataframe(train_set, full_features, mode='train')
    df_VAL = data.organize_dataframe(val_set, full_features, mode='val')
    
    ### Train models ###
    linReg, quadReg, cubReg, rF, nN, sScaler, train_set, val_set = train.train_models(features, df_TRAIN, df_VAL, train_set, val_set, all_sites_df_TRAIN, all_sites_df_VAL, save_path)
    
    # Combine model predictions to single dataframe
    all_sites_df_TRAIN = all_sites_df_TRAIN.merge(train_set[[f'pred_{config.output}_LM',f'pred_{config.output}_P2M',f'pred_{config.output}_P3M',f'pred_{config.output}_RF',f'pred_{config.output}_NN']],how='left', left_index=True, right_index=True)
    all_sites_df_VAL = all_sites_df_VAL.merge(val_set[[f'pred_{config.output}_LM',f'pred_{config.output}_P2M',f'pred_{config.output}_P3M',f'pred_{config.output}_RF',f'pred_{config.output}_NN']],how='left', left_index=True, right_index=True)
